...
- [x] Register `IProjectService` in `src/Stretto.Api/Program.cs`: add `builder.Services.AddScoped<IProjectService, ProjectService>();` after the existing `AddScoped<VenueService>()` line; add `using Stretto.Application.Interfaces;` and `using Stretto.Application.Services;` if not already present
