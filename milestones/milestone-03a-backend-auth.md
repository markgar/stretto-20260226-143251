...
- [x] Update `Stretto.Api/Program.cs`: add `builder.Services.AddControllers()`; add `builder.Services.AddSingleton<IAuthSessionStore, InMemoryAuthSessionStore>()`; add `builder.Services.AddScoped<AuthService>()`; call `app.MapControllers()` after the health endpoint; add the required `using` statements for the new namespaces (`Stretto.Application.Services`, `Stretto.Application.Interfaces`, `Stretto.Infrastructure.Auth`)
