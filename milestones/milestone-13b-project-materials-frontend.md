...
- [ ] Add the documents list to `ProjectMaterialsTab`: render each document as a row with the title text, a download anchor `<a href={`/api/projects/${projectId}/documents/${doc.id}/download`} download={doc.fileName} data-testid={`download-document-${doc.id}`}>Download</a>`, and a delete button `data-testid={`delete-document-${doc.id}`}` (admin only) that calls `ProjectMaterialsService.deleteApiProjectsDocuments(projectId, doc.id)` via `useMutation` and invalidates `['projectDocuments', projectId]` on success
