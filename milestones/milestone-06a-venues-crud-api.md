...
- [x] Register `VenueService` in `src/Stretto.Api/Program.cs`: add `builder.Services.AddScoped<VenueService>();` after the existing `AddScoped<AuthService>()` line

- [x] Create `src/Stretto.Api/Controllers/VenuesController.cs` with `[ApiController]`, `[Route("api/venues")]`; constructor-inject `VenueService` and `AuthService`; add private async helper `GetOrgIdAsync()` that reads `Request.Cookies["stretto_session"]`, throws `UnauthorizedException` if null, calls `AuthService.ValidateAsync(token)`, and returns `dto.OrgId`; implement: `GET /api/venues` → calls `GetOrgIdAsync()` then `VenueService.ListAsync(orgId)`, returns `Ok(list)`; `GET /api/venues/{id}` → returns `Ok(dto)` or propagates `NotFoundException`; `POST /api/venues` → creates venue, returns `Created($"/api/venues/{dto.Id}", dto)`; `PUT /api/venues/{id}` → returns `Ok(dto)`; `DELETE /api/venues/{id}` → returns `NoContent()`
